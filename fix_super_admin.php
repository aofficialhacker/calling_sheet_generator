...
$result = $conn->query("SELECT admin_id FROM admin_users WHERE admin_id = 'SUPER'");
if ($result && $result->num_rows > 0) {
    ...
    $query = "INSERT INTO admin_users (admin_id, username, name, password, designation, is_active) VALUES ('SUPER', 'system_super', 'System Superadmin', '" . password_hash('super123', PASSWORD_DEFAULT) . "', 'Superadmin', 1)";
...
if ($stmt->execute()) {
    ...
    $conn->query("DELETE FROM team_leader_dispositions WHERE id = $testId");
...
echo "<p>The issue was that the foreign key constraint required 'SUPER' to exist in admin_users table.</p>";

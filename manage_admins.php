...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'create_admin') {
        ...
        $stmt = $conn->prepare("
            SELECT username, password, name, designation 
        ...
        if ($result->num_rows > 0) {
            ...
            if ($checkStmt->get_result()->num_rows > 0) {
                ...
                if ($insertStmt->execute()) {
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        $vendorId = generateVendorId($adminId, $conn);
                        $vendorName = "Default Vendor $i";
                        
                        $vendorStmt = $conn->prepare("INSERT INTO vendors (vendor_id, vendor_name, admin_id) VALUES (?, ?, ?)");
                        $vendorStmt->bind_param("sss", $vendorId, $vendorName, $adminId);
                        $vendorStmt->execute();
                        $vendorStmt->close();
                    }
                    
                    $message = "Admin user created successfully with ID: $adminId";
                } else {
...
$admins = $conn->query("
    SELECT au.*, 
           ...
           (SELECT COUNT(DISTINCT fcl.finqy_id) 
            ...
            JOIN admin_caller_mapping acm ON fcl.finqy_id = acm.finqy_id COLLATE utf8mb4_general_ci 
            WHERE acm.admin_id = au.admin_id) as caller_count,
                                    ...
                                    <?php if ($admins && $admins->num_rows > 0): ?>
                                        <?php while($admin = $admins->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="admin_details.php?id=<?= $admin['admin_id'] ?>" class="btn btn-sm btn-info text-white">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                    <a href="assign_callers.php?id=<?= $admin['admin_id'] ?>" class="btn btn-sm btn-warning">
            ...
            <div class="modal-content">
                ...
                <form method="POST">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="username" class="form-label">Select Corporate User</label>

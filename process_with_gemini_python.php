...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['markedSheet'])) {
    ...
    if (move_uploaded_file($_FILES['markedSheet']['tmp_name'], $targetFile)) {
        // Execute Python script and capture CSV output
        $command = "python gemini_omr_parser.py " . escapeshellarg($targetFile);
        $output = shell_exec($command);

        // Check for valid CSV output
        $lines = explode("\n", trim($output));
        $headers = !empty($lines) ? str_getcsv(array_shift($lines)) : false;

        if (!$headers || (isset($headers[0]) && strtolower(trim($headers[0])) !== 'unique_id')) {
            ...
            if ($error_check && isset($error_check['error'])) {
                 ...
                 $error = "AI script returned an invalid header format. Expected 'unique_id' as first column. Response: <pre>" . htmlspecialchars($output) . "</pre>";
            }
        } else {
            // If CSV is valid, fetch customer names
            $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
            if ($conn->connect_error) { die("DB Connection Failed."); }

            $stmt = $conn->prepare("SELECT name FROM temp_processed_data WHERE unique_id = ?");
            ...
            foreach ($lines as $line) {
                ...
                if (count($row_data) === count($headers)) {
                    ...
                    $unique_id = $parsed_data['unique_id'] ?? null;
                    if (!empty($unique_id)) {
                        $stmt->bind_param("s", $unique_id);
                        $stmt->execute();
                        $res = $stmt->get_result();
                        $customer = $res->fetch_assoc();
                        $parsed_data['customer_name'] = $customer['name'] ?? 'Unknown ID';
                        $results[] = $parsed_data;
                    }
                }
            }
            ...
            // If the script ran but found no valid rows, set a user-friendly error.
            if (empty($results)) {
...
<div class="container mt-4 mb-5">
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <p class="text-muted">Capture a photo of the sheet, ensuring the "Unique id", "Connectivity", and "Disposition" columns are clear.</p>
    ...
    <div class="card mt-5 shadow-sm">
        ...
        <div class="card-body">
            <table class="table table-bordered table-striped">
                <thead><tr><th>Customer Name</th><th>Connectivity</th><th>Disposition</th></tr></thead>
                <tbody>
                    <?php foreach ($results as $row): ?>
                        <tr>
                            <td><?= htmlspecialchars($row['customer_name']) ?><small class="text-muted d-block"><?= htmlspecialchars($row['unique_id']) ?></small></td>

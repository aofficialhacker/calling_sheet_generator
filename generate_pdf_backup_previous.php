...
class CallSheetPDF extends TCPDF {
    ...
    public function __construct($title, $slotLegend, $dispLegend, $finalHeaders) {
        ...
        // More accurate positioning based on actual column widths
        $this->cutlinePosition = 10; // Start from left margin
        for ($i = 0; $i <= $mobileIndex; $i++) {
            switch ($finalHeaders[$i]) {
                case 'id': $this->cutlinePosition += 20; break;
                case 'slot': $this->cutlinePosition += 12; break;
                case 'connectivity': $this->cutlinePosition += 25; break;
                case 'disposition': $this->cutlinePosition += 45; break;
                case 'mobile_no': $this->cutlinePosition += 28; break;
                default: $this->cutlinePosition += 20; break;
            }
        }
    }
    ...
    public function drawTableHeader($finalHeaders) {
        ...
        // Calculate total available width (A4 landscape minus margins)
        $totalWidth = 277; // A4 landscape width (297mm) minus left/right margins (10mm each)
        ...
        foreach ($finalHeaders as $header) {
            switch ($header) {
                case 'id': $widths[] = 20; break; // ID needs to be readable
                case 'slot': $widths[] = 12; break; // Small, just for checkboxes
                case 'connectivity': $widths[] = 25; break; // "○ Y / ○ N"
                case 'disposition': $widths[] = 45; break; // INCREASED - needs space for multiple codes
                case 'mobile_no': $widths[] = 28; break; // Mobile numbers are important
                case 'name': $widths[] = 35; break; // Names can be long
                case 'title': $widths[] = 18; break; // Mr./Mrs./etc.
                case 'policy_number': $widths[] = 25; break; // Policy numbers
                case 'age': $widths[] = 12; break; // Small number
                case 'dob': $widths[] = 22; break; // Date format
                case 'expiry': $widths[] = 22; break; // Date format
                case 'address': $widths[] = 40; break; // Addresses are long
                case 'city': $widths[] = 25; break; // City names
                case 'state': $widths[] = 25; break; // State names
                default: $widths[] = 20; break; // Default for other columns
            }
        }
        ...
        if ($currentTotal > $totalWidth) {
            $scaleFactor = $totalWidth / $currentTotal;
            $widths = array_map(function($w) use ($scaleFactor) { return $w * $scaleFactor; }, $widths);
        }
...
try {
    ...
    while ($rowsProcessed < $totalRecords) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $rowHeight = 12; // Increased from 6 to accommodate wrapped text
            
            foreach ($finalHeaders as $i => $header) {
                $cellContent = '';
                $fontSize = 8; // Larger default font
                $fontStyle = '';
                
                switch($header) {
                    case 'disposition':
                        // Create compact disposition grid on single line
                        $dispCodes = [];
                        foreach ($dispositionList as $disp) {
                            $dispCodes[] = 'O ' . $disp['code']; // Use 'O' instead of unicode
                        }
                        $cellContent = implode(' ', array_slice($dispCodes, 0, 8)); // First 8 codes
                        if (count($dispositionList) > 8) {
                            $cellContent .= "\n" . implode(' ', array_slice($dispCodes, 8)); // Remaining codes
                        }
                        $fontSize = 7; // Larger font for disposition codes
                        break;
                    case 'connectivity':
                        $cellContent = 'O Y / O N'; // Use 'O' instead of unicode circle
                        break;
                    case 'slot':
                        $cellContent = ''; // Empty for manual entry
                        break;
                    case 'mobile_no':
                        $cellContent = $row[$header] ?? '';
                        $fontStyle = 'B'; // Bold for mobile
                        $fontSize = 8; // Larger font for mobile
                        break;
                    case 'id':
                        $cellContent = $row[$header] ?? ''; // Show FULL ID, no truncation
                        $fontSize = 7; // Readable font for ID
                        break;
                    case 'dob':
                    case 'expiry':
                        ...
                        break;
                    case 'address':
                        // Truncate long addresses and add line breaks
                        $addr = $row[$header] ?? '';
                        if (strlen($addr) > 30) {
                            $cellContent = substr($addr, 0, 30) . "\n" . substr($addr, 30, 20) . (strlen($addr) > 50 ? '...' : '');
                        } else {
                            $cellContent = $addr;
                        }
                        $fontSize = 5;
                ...
                // Draw cell border manually for MultiCell
                $pdf->Rect($currentX, $currentY, $columnWidths[$i], $rowHeight, 'D');
                
                // Add text with proper alignment
                $pdf->SetXY($currentX + 1, $currentY + 1); // Small padding
                $pdf->MultiCell($columnWidths[$i] - 2, 4, $cellContent, 0, 'L', false);
                
                // Move to next column position
                $pdf->SetXY($currentX + $columnWidths[$i], $currentY);
            }
            
            // Move to next row
            $pdf->SetXY(10, $pdf->GetY() + $rowHeight);
            $rowsProcessed++;
        }
